...
impl SquareRandProofCommitments {
    ...
    pub fn from_bytes(slice: &[u8]) -> Result<SquareRandProofCommitments, SquareRandProofCommitmentsError> {
        if slice.len() != 3 * 32 {
            return Err(SquareRandProofCommitmentsError::FormatError);
        }
        let eg = ElGamalPair::from_bytes(&slice[0..2*32]).unwrap();
        let ped = CompressedRistretto(read32(&slice[2*32..])).decompress().ok_or(SquareRandProofCommitmentsError::FormatError)?;
