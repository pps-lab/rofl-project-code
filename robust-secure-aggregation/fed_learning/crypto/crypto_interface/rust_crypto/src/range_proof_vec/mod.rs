...
pub fn create_rangeproof(value_vec_clipped: &Vec<f32>, blinding_vec: &Vec<Scalar>, prove_range: usize, n_partition: usize)
-> Result<(Vec<RangeProof>, Vec<RistrettoPoint>), RangeProofError> {
    ...
    // I LOVE ITERATORS!!!
    let value_fp_vec_clipped_shifted: Vec<URawFix> = 
    value_vec_clipped.par_iter()
    ...
    let rp_vec = compute_shifted_values_rp(&rp_vec_shifted[0..value_vec_clipped.len()], &inv_offset_rp);
...
pub fn verify_rangeproof(range_proof_vec: &Vec<RangeProof>, commit_vec: &Vec<RistrettoPoint>, prove_range: usize) 
-> Result<bool, ProofError>{
    ...
    let commit_vec_shifted_ext: Vec<RistrettoPoint> = extend_vec_to_pow2(&commit_vec_shifted, rp_zero);
...
mod tests {
    ...
    fn test_rangeproof_roundtrip() {
        ...
        let (range_proof_vec, commit_vec) = create_rangeproof(&values, &blindings, N_BITS, prove_range).unwrap();
    ...
    fn test_fake_proof() {
        ...
        let blindings: Vec<Scalar> = rnd_scalar_vec(values.len());
        let (range_proof_vec, _) = create_rangeproof(&values, &blindings, N_BITS, prove_range).unwrap();
    ...
    fn test_rangeproof_with_cancelling_blindings() {
        ...
        let (range_proof_x, commitments_x) = create_rangeproof(&x_vec, &blindings[0], range_exp, n_partition).unwrap();
        let (range_proof_y, commitments_y) = create_rangeproof(&y_vec, &blindings[1], range_exp, n_partition).unwrap();
        let (range_proof_z, commitments_z) = create_rangeproof(&z_vec, &blindings[2], range_exp, n_partition).unwrap();
        ...
        let sum_vec_rp: Vec<RistrettoPoint> = add_rp_vec_vec(&vec![commitments_x.to_vec(), commitments_y.to_vec(), commitments_z.to_vec()]);
    ...
    fn test_rangeproof_clipped() {
        ...
        let (_, commitments_x) = create_rangeproof(&x_vec_clipped, &blindings, prove_range, n_partition).unwrap();
