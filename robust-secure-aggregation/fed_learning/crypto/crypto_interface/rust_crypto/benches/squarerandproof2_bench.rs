...
fn createproof_label(dim: usize) -> String{
    ...
    format!("create-squarerandproof-{:02}-{:05}-({})",
        N_BITS,
        dim,
...
fn verifyproof_label(dim: usize) -> String{
    let t: DateTime<Local> = Local::now();
    format!("verify-squarerandproof-{:02}-{:05}-({})",
        N_BITS,
        dim,
        t.format("%Y-%m-%d-%H-%M-%S").to_string())
}
...
benchmark_group!(bench_squarerandproof2, bench_squarerandproof_fn);
benchmark_main!(bench_squarerandproof2);