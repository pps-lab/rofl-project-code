...
def build_model(model_config: GlobalModelConfig, test_data):
    return CIFARLeNetSubspace(model_config, test_data)

def getType():
    return CIFARLeNetSubspace

SUBSPACE_VSIZE = 15000
SEED = 41

class CIFARLeNetSubspace(GlobalModel):
    
    def __init__(self, model_config: GlobalModelConfig, test_data):
        super(CIFARLeNetSubspace, self).__init__(model_config, test_data)
    ...
    def build_model(cls, seed=None):
        ...
        xx = RProjConv2D(proj_type, 6, kernel_size=5, strides=1,  kernel_initializer='he_normal', padding='valid', activation='relu', kernel_regularizer=l2(weight_decay))(preproc_images)
        xx = MaxPooling2D((2, 2))(xx)
        xx = RProjConv2D(proj_type, 16, kernel_size=5, strides=1,  kernel_initializer='he_normal', padding='valid', activation='relu', kernel_regularizer=l2(weight_decay))(xx)
        xx = MaxPooling2D((2, 2))(xx)
        xx = Flatten()(xx)
        xx = RProjDense(proj_type, 120, kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(weight_decay))(xx)
        xx = RProjDense(proj_type, 84, kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(weight_decay))(xx)
    ...
    def serialize_model_name(self):
        return "test.demo.cifar_lenet_subspace"
