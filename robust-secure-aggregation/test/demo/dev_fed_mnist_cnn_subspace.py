...
class DevFedMNISTCNNSubspace(GlobalModel):
    ...
    def build_model(cls, seed=None):
        ...
        xx = RProjConv2D(proj_type, 8, kernel_size=3, strides=1,  kernel_initializer='he_normal', padding='valid', activation='relu', kernel_regularizer=l2(weight_decay))(preproc_images)
        xx = RProjConv2D(proj_type, 4, kernel_size=3, strides=1,  kernel_initializer='he_normal', padding='valid', activation='relu', kernel_regularizer=l2(weight_decay))(xx)
        xx = MaxPooling2D((2, 2))(xx)
        xx = Flatten()(xx)
        xx = RProjDense(proj_type, 32, kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(weight_decay))(xx)
    ...
    def serialize_model_name(self):
        return "test.demo.dev_fed_mnist_cnn_subspace"
