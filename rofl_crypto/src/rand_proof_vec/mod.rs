...
pub fn create_randproof_vec(
    ...
    let res_vec: Vec<Result<(RandProof, ElGamalPair), ProofError>> = randproof_args
        .par_iter()
        .map(|(x, r)| RandProof::prove(&eg_gens, &mut Transcript::new(b"RandProof"), **x, **r))
        .collect();
...
pub fn create_randproof_vec_existing(
    ...
    let randproof_args: Vec<((&Scalar, &Scalar), &RistrettoPoint)> = value_scalar_vec
        .iter()
        .zip(random_vec)
        .zip(&existing_value_com_vec)
        .collect();

    let res_vec: Vec<Result<(RandProof, ElGamalPair), ProofError>> = randproof_args
        .par_iter()
        .map(|((x, r), p)| {
            RandProof::prove_existing(&eg_gens, &mut Transcript::new(b"RandProof"), **x, **p, **r)
        })
        .collect();
...
pub fn verify_randproof_vec(
    ...
    let eg_gens: ElGamalGens = ElGamalGens::default();
    let verify_args: Vec<(&RandProof, &ElGamalPair)> =
        randproof_vec.par_iter().zip(commit_vec).collect();
    let res_vec: Vec<Result<(), ProofError>> = verify_args
        .par_iter()
        .map(|(rp, c)| rp.verify(&eg_gens, &mut Transcript::new(b"RandProof"), **c))
        .collect();
