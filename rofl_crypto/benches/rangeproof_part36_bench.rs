...
fn createproof_label(dim: usize, range: usize) -> String {
    ...
    format!(
        "create-rangeproof-part36-{:02}-{:02}-{:05}-({})",
        N_BITS,
        range,
        dim,
...
fn verifyproof_label(dim: usize, range: usize) -> String {
    let t: DateTime<Local> = Local::now();
    format!(
        "verify-rangeproof-part36-{:02}-{:02}-{:05}-({})",
        N_BITS,
        range,
        dim,
        t.format("%Y-%m-%d-%H-%M-%S").to_string()
...
benchmark_group!(rangeproof_part36_bench, bench_rangeproof_part36_fn);
benchmark_main!(rangeproof_part36_bench);
