...
leonhard_train() {
    ...
    module load gcc/6.3.0 python/3.7.4 hdf5/1.10.1
    pip install --user -r requirements.txt
    experiment_name="$1"
    outfile="lsf.%J.$experiment_name.out"
    job=$(bsub -W 1440 -n 20 -J $experiment_name -oo $outfile -R "rusage[mem=4096]" -N "python -m src.main --experiment_name $experiment_name ${@:2}")
...
ssh $username@login.euler.ethz.ch << EOF
    $(typeset -f)
    leonhard_train $@
EOF
