...
import pandas as pd
from baseline import print_table
...
def make_plots(dfs):
    ...
    for obj in dfs:
        ...
        if not plot_single:
            ...
            file_path = join('.', 'combined_adv_acc_plots', f'{experiment_name}_{attack_type}_{dataset}_{number_of_samples}.pdf')
    ...
    if plot_single:
        ...
        file_path = join('.', 'combined_adv_acc_plots',
                         f'combined_all.pdf')
        plt.savefig(file_path)
        plt.show()
...
def get_df_list(directory_wildcard='hist*', experiment_name = ""):
    ...
    dirs = glob(join(log_dir, experiment_name))
