...
for dataset in "${dataset_list[@]}"; do
    for clients in "${clients_list[@]}"; do
        for scale_attack in "${scale_attack_list[@]}"; do
            experiment_name=${attack_type}_${untargeted_after_training}_mal_baseline_boosting_${scale_attack}_num_malicious_clients_${num_malicious_clients}_${dataset}_clients_${clients}
            cmd="python -m src.main --attack_type ${attack_type} --untargeted_after_training ${untargeted_after_training} --num_malicious_clients ${num_malicious_clients} --scale_attack ${scale_attack} --scale_attack_weight ${clients} --dataset ${dataset} --num_clients ${clients} --model_name ${model_name} --experiment_name ${experiment_name}"
            echo "$cmd"
            set -x
            ${cmd}
            set +x
