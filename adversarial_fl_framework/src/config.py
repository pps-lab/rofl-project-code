...
parser.add_argument('--attack_type', type=str, default='untargeted', help='Attack type.',
                    choices=['untargeted', 'targeted', 'targeted_deterministic', 'min_loss', 'data_poison'])
...
parser.add_argument('--backdoor_type', type=str, default='semantic', help='Backdoor type.',
                    choices=['semantic', 'trigger'])
...
parser.add_argument('--backdoor_attack_objective', type=int, default=[7, 1], nargs='+',
                    help="What class to mispredict `aux_samples` times. Only applicable if num_malicious_clients is non-zero value and 'attack_type' is 'min_loss'.")
...
parser.add_argument('--backdoor_feature_aux_train', type=int, default=[], nargs='+',
                    help="What samples to use as aux train set. Only applicable 'attack_type' is 'min_loss' or 'data_poison'.")
parser.add_argument('--backdoor_feature_aux_test', type=int, default=[], nargs='+',
                    help="What samples to use as aux test set. Only applicable 'attack_type' is 'min_loss' or 'data_poison'.")
...
parser.add_argument('--pgd_adaptive', type=str, default="false", help="Whether to be adaptive in the gradient clipping.")
...
parser.add_argument("--model_name", type=str, default='dev', help="Which model to use.",
                    choices=['dev', 'mnist_cnn', 'bhagoji', 'resnet18', 'resnet32', 'resnet44', 'resnet56', 'resnet110', 'resnet18_v2', 'resnet56_v2', 'dev_intrinsic', 'dev_fc_intrinsic', 'bhagoji_intrinsic', 'mnistcnn_intrinsic', 'lenet5', 'lenet5_intrinsic', 'allcnn', 'allcnn_intrinsic'])
...
parser.add_argument('--learning_rate', type=float, default=0.0001, help='Learning rate for selected optimizer.')
...
def get_config():
    ...
    config['mal_learning_rate'] = args.mal_learning_rate[0] if len(args.mal_learning_rate) > 0 else args.learning_rate
    ...
    config['federated_dropout_rate'] = args.federated_dropout_rate
    config['federated_dropout_all_parameters'] = args.federated_dropout_all_parameters
    config['federated_dropout_nonoverlap'] = args.federated_dropout_nonoverlap
    config['federated_dropout_randommask'] = True if args.federated_dropout_randommask.lower() == "true" else False
    ...
    assert 0 < args.federated_dropout_rate <= 1, 'Federated dropout rate must be in (0, 1] range.'
