...
class GlobalDataset:
    ...
    def get_test_batch(self, batch_size, max_num_batches):
...
# TODO 0: Do we want to use num_backdoor_tasks or aux_sample_size ?
    def build_global_aux(self, mal_clients, num_backdoor_tasks, attack_objective, aux_sample_size, augment_size):
        ...
        # TODO 1: Do we take from train or test? (i.e. bhagoji vs. google)
        ...
        for i in range(len(data_x)):
            ...
            if mal_clients[i]:
                ...
                print(f"{i} Found {len(inds)} of class {attack_objective} to poison!")
        ...
        self.mal_aux_labels_train = np.concatenate(total_mal_aux_labels)
    ...
    def get_full_dataset(self):
        return np.concatenate(self.x_train), np.concatenate(self.y_train)
...
class DirichletDistributionDivider():
    ...
    def __init__(self, x_train, y_train, train_aux, test_aux, num_clients):
    ...
    def build(self):
        ...
        for ind, x in enumerate(self.y_train):
            label = x
            if ind in self.train_aux or ind in self.test_aux:
                continue
