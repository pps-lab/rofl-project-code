...
impl FlTraining {
    pub async fn train_for_round(&mut self, config : ModelConfig, params : Vec<f32>, round_id : i32, model_id : i32) -> Option<Vec<f32>> {
        match self {
            FlTraining::Dummy => {
                return Some(vec![0.001; params.len()]);
            }
            FlTraining::Grpc(client) => {
                return client.train_for_round(config, params, round_id, model_id).await;
            }
        }
    }
}
...
impl FlTrainClient {
    pub async fn train_for_round(&mut self, config : ModelConfig, params : Vec<f32>, round_id : i32, model_id : i32) -> Option<Vec<f32>> {
        ...
        tokio::spawn(async move {
            ...
            let  num_blocks = if num_elems % NUM_FLOATS_PACKET == 0 { num_elems / NUM_FLOATS_PACKET } 
            else { num_elems / NUM_FLOATS_PACKET + 1 };
            let _res = outbound_local.send(request).await;   
            let request = ClientModelMessage {
                model_message : Some(client_model_message::ModelMessage::MetaBlockMessage(MetaFloatBlockMessage {
                    model_id: model_id,
                    round_id: round_id,
            ...
            for block_id in 0..num_blocks {
                ...
                let request = ClientModelMessage {
                    model_message : Some(client_model_message::ModelMessage::ModelBlock(FloatBlock {
