...
pub fn create_l2rangeproof_vec_existing(
    ...
    let res_vec: Vec<Result<(SquareRandProof, SquareRandProofCommitments), ProofError>> =
        randproof_args_1
            .par_iter()
            .map(|(((x, r_1), r_2), m_com)| {
                SquareRandProof::prove_existing(
                    &eg_gens,
                    &mut Transcript::new(b"SquareRandProof"),
                    **x,
                    **m_com,
                    **r_1,
                    **r_2,
    ...
    let mut commitments_pair_vec: Vec<SquareRandProofCommitments> =
...
pub fn verify_l2rangeproof_vec(
    ...
    let eg_gens: ElGamalGens = ElGamalGens::default();
    let verify_args: Vec<(&SquareRandProof, &SquareRandProofCommitments)> =
        randproof_vec.iter().zip(commit_vec).collect();
    let res_vec: Vec<Result<(), ProofError>> = verify_args
        .par_iter()
        .map(|(rp, c)| rp.verify(&eg_gens, &mut Transcript::new(b"SquareRandProof"), **c))
        .collect();
